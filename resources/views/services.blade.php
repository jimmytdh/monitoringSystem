...
@section('body')
    <h2 class="text-success title-header">Services <small class="text-muted">Panel</small></h2>
            ...
            <div class="col-md-4">
                ...
                <div class="box box-primary">
                    @if(!$edit)
                        ...
                        <!-- /.box-header -->
                        <!-- form start -->
                        <form role="form" method="post" action="{{ url('/library/services/save') }}">
                            {{ csrf_field() }}
                            <div class="box-body">
                                <div class="form-group">
                                    <select name="section" id="" class="form-control">
                                        @foreach($section as $s)
                                            <option>{{ strtoupper($s) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                    ...
                    @else
                        ...
                        <!-- /.box-header -->
                        <!-- form start -->
                        <form role="form" method="post" action="{{ url('/library/services/'.$info->id) }}">
                            {{ csrf_field() }}
                            <div class="box-body">
                                <div class="form-group">
                                    <select name="section" id="" class="form-control">
                                        @foreach($section as $s)
                                            <option @if($info->section==$s) selected @endif>{{ strtoupper($s) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <input type="text" class="form-control" value="{{ $info->name }}" required name="name" placeholder="Enter Service Name...">
                                </div>
                                <div class="form-group">
                                    <select name="type" id="" class="form-control">
                                        <option value="">None</option>
                            ...
                            <div class="box-footer">
                                ...
                                <a href="{{ url('/library/services/delete/'.$info->id) }}" class="btn btn-danger btn-block btn-delete">Delete</a>
                                <a href="{{ url('/library/services') }}" class="btn btn-default btn-block">Back</a>
                            </div>
                        </form>
                    @endif
                </div>
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <div class="pull-right">
                            <form action="{{ url('/library/services/search') }}" method="post">
                                {{ csrf_field() }}
                                <div class="input-group input-group-sm mb-0">
                                    <input type="text" class="form-control" name="keyword" value="{{ Session::get('serviceKeyword') }}" aria-label="Small" aria-describedby="inputGroup-sizing-sm">
